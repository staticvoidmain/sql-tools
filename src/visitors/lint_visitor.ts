...
type SpellFunction = (word: string) => boolean;

let isMisspelled: SpellFunction;

try {
  let spellchecker = require("spellchecker");

  // todo: this is hacky, allow user dictionaries
  spellchecker.add("cte");
  spellchecker.add("ctas");
  spellchecker.add("crtas");
  isMisspelled = spellchecker.isMisspelled;
  feature.spell = true;
} catch {}
...
export class ExampleLintVisitor extends Visitor {
  ...
  private emit(
    ...
    console.log(
      `${file}:${line + 1}:${col + 1} - ${sev} -${category}${message}\n`
    );
  ...
  visitSelect(node: SelectStatement) {
    const multipleSources =
      node.from && (node.from.sources.length > 1 || node.from.joins);
        ...
        walkExpr(col, (expr) => {
          if (expr.kind === SyntaxKind.identifier_expr) {
            ...
            if (parts.length < 2) {
              ...
              // the real rule is tables should be schema qualified
              this.warning(
                "use a two-part name for queries involving more than one table",
                ident
              );
            }
          }
  ...
  visitColumnExpression(node: ColumnExpression) {
    if (node.style === "alias_equals_expr") {
      this.info(
        '"alias = expression" syntax is deprecated, use "expression as alias" instead.',
        node
      );
    }
  }
  ...
  visitBinaryExpression(node: BinaryExpression) {
    if (isNullLiteral(node.left) || isNullLiteral(node.right)) {
      this.error(
        'null literal used in comparison, use "is null" or "is not null" instead',
        node,
        node.op
      );
    } else {
      if (isComparison(node.op) && exprEquals(node.left, node.right)) {
        ...
        this.error(
          "value compared with itself, result will always be constant",
          node
        );
      }
    }
    ...
    // rule: expressions in a divisor slot which are non-literal, could be div by zero
  }
  ...
  visitLike(like: LikeExpression) {
    ...
    if (!hasLeadingPrefix(pattern)) {
      this.warning(
        "patterns which do not begin with a prefix cannot benefit from indexes",
        like
      );
    }
  ...
  visitDrop(drop: DropStatement) {
    if (drop.objectType.kind === SyntaxKind.table_keyword) {
      if (isTemp(drop.target)) {
        ...
        if (i === -1) {
          this.info(
            "Dropping a temp table you didn't create in this file.",
            drop,
            undefined,
            "code-smell"
          );
  ...
  visitEndOfFile() {
    if (this.tempTables.length > 0) {
      ...
      const message =
        "One or more #temp_tables exist after end of script: " + tables;

      this.info(
        message,
        new Token(SyntaxKind.EOF, 0, 0),
        undefined,
        "code-smell"
      );
    }
