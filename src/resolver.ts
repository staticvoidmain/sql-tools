...
some notes:
  ...
  this can be replaced with a "GO" statement or other batch separator,
  like ; for postgres.
...
interface LocalTable extends Entity {
  readonly kind: SymbolKind.local_table;
}

// hmm... is this what I meant?
interface Subquery extends Entity {
  readonly kind: SymbolKind.query;
}

interface Database extends Entity {
  readonly kind: SymbolKind.database;
}

interface Schema extends Entity {
  readonly kind: SymbolKind.schema;
}
...
interface View extends Entity {
  // set by the resolver if the view could be used in an
  // update statement. requires processing the view declaration
  // after parsing
  updateable: boolean;
}
...
class NameTable {
  ...
  add(name: string, sym: Symbol) {
    // hmm... maybe an array of symbols to support naming the
    // collision later...?
...
export function configureResolver(opt: Partial<ResolverOptions>) {
...
export class Scope {
  ...
  constructor(private parent_scope?: Scope, private name?: string) {
    if (parent_scope) {
  ...
  define(entity: Entity) {
    if (!options.allowShadow) {
      if (this.parent_scope) {
        ...
        if (sym) {
          // todo warn('symbol already defined in a parent scope')
        }
      }
    }
...
function resolveExpr(scope: Scope, expr: Expr) {
  switch (expr.kind) {
    ...
    case SyntaxKind.binary_expr: {
      ...
      // todo: type the resulting binary expr also
      break;
    }
...
function resolveSelectStatement(batch: Scope, node: SyntaxNode) {
  ...
  if (select.ctes) {
    for (const cte of select.ctes) {
      // should this just be resolveExpr?
  ...
  if (select.from) {
    for (const src of select.from.sources) {
      if (src.expr.kind === SyntaxKind.identifier_expr) {
        ...
        if (src.alias) {
          selectScope.define(alias(last(src.alias.parts), entity));
        }
      ...
      if (src.alias) {
        if (src.kind === SyntaxKind.select_statement) {
          const query = <SelectStatement>src.expr;
          const sub = selectScope.createScope("sub-select");

          // recurse down, not sure this is the best way
          // to handle this, but w/e
          resolveSelectStatement(sub, query);

          const table_expr: Subquery = {
            name: last(src.alias.parts),
            kind: SymbolKind.query,
          };

          selectScope.define(table_expr);
          break;
        }
...
// this is meant to be called from a top-level
export function resolveAll(nodes: SyntaxNode[], scope: Scope): void {
  // todo: keep a list of these scopes, currently we're just abandoning
  const batches = [];

  let batchCounter = 0;
  let batch = scope.createScope("batch_" + batchCounter);
  ...
  for (const node of nodes) {
    switch (node.kind) {
      case SyntaxKind.go_statement: {
        batchCounter++;
        batch = scope.createScope("batch_" + batchCounter);
        batches.push(batch);
        break;
      }

      case SyntaxKind.create_table_statement: {
        // todo: declare temp tables
        break;
      }
      ...
      case SyntaxKind.set_statement: {
        ...
        if (!decl) {
          throw Error(`undeclared identifier ${set.name}`);
        }
      ...
      case SyntaxKind.select_statement: {
        resolveSelectStatement(batch, node);
        break;
      }
...
// undefined symbols resolve to some "unknown" type
// that taints things upward.
export function loadEnvironment(file: string) {
  ...
  for (const dbName in json.databases) {
    ...
    loadDatabase(scope.createScope(dbName), json.databases[dbName]);
  }
